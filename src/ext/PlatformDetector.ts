...
export class PlatformDetector {
    private _rules: PlatformDetectorRule[] = [
    ];

    constructor() {
        this._rules.push(new WPFXamlRule());
        this._rules.push(new XamarinXamlRule());
        this._rules.push(new AngularHTMLRule());
        this._rules.push(new ReactHTMLRule());
        this._rules.push(new WebComponentsHTMLRule());
        this._rules.push(new AngularTSRule());
        this._rules.push(new ReactTSRule());
        this._rules.push(new WebComponentsTSRule());
        this._rules.push(new BlazorRule());
    }

    detectPlatform(fencedBlockInfo: FencedBlockInfo): APIPlatform[] {
        let plats: APIPlatform[] = [];
        for (let rule of this._rules) {
            if (rule.match(fencedBlockInfo)) {
                plats.push(rule.platform);
            }
        }
        return plats;
    }
