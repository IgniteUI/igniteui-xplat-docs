...
The {ProductName} State Persistence in {Platform} {ComponentTitle} allows developers to easily save and restore the grid state. When the `GridState` is applied on the {Platform} `Grid`, it exposes the `GetState` and `SetState` methods that developers can use to achieve state persistence in any scenario.
