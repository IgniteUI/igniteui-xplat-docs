...
Then define a {ComponentTitle} with bound data source, `RowEditable` set to true and an [Action Strip](../action-strip.md) component with editing actions enabled. The `AddRow` input controls the visibility of the button that spawns the row adding UI.
