...
```html
<igc-chat id="myChat" .options="${options}"> </igc-chat>
```
...
return <IgrChat options={options} />;
```
...
const ChatExample = () => {
  ...
  return (
    <>
      <IgrChat
        ref={chatRef}
        options={{ currentUserId: "me", headerText: "Support Chat" }}
      />
...
```html
<igc-chat .options="${options}"> </igc-chat>
```
...
return <IgrChat ref={chatRef} options={{ options }} />;
```
...
const options = {
  renderers: {
    messageContent: (ctx) => {
      const { message } = ctx;
      return <div className="bubble custom">${message.content}</div>;
    },
  },
};
```
...
const options = {
  renderers: {
    input: (ctx) => html`
      <textarea
        placeholder=${ctx.instance?.options?.inputPlaceholder || "Type here..."}
      >
${ctx.value}</textarea
      >
    ...
    input: (ctx) => {
      return (
        <>
          <textarea
            placeholder={
              ctx.instance?.options?.inputPlaceholder || "Type here..."
            }
          >
            {ctx.value}
          </textarea>
...
const _actionsEndTemplate = (ctx) => (
  <div>
    <IgrButton onClick={() => handleCustomSendClick(ctx.instance)}>
      Ask
    </IgrButton>
