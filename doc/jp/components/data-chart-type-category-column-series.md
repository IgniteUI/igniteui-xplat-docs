...
$ProductName$ 縦棒チャートはカテゴリ チャートのグループに属し、チャートの下から上へデータポイントの値に向かって伸びる長方形のコレクションを使用して描画されます。`ColumnSeries` は時間毎のデータの変化や複数の項目を比較する場合に用いられます。値は y 軸に表示され、カテゴリは x 軸に表示されます。`ColumnSeries` は `BarSeries` と同じデータプロットの概念を使用しますが、データポイントは垂直線 (y 軸) ではなく水平線 (x軸) に沿って引き伸ばされます。つまり、縦棒シリーズは棒シリーズのように描画されますが、反時計回りに 90 度回転されます。
