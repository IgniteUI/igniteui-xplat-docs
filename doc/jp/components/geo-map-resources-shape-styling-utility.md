---
title: $PlatformShort$ マップ | シェープ マップのリソース | インフラジスティックス
_description: インフラジスティックスの $PlatformShort$ JavaScript マップを使用して、シェイプ ファイルから地理空間データを読み込みます。$ProductName$ マップのサンプルを是非お試しください!
_keywords: $PlatformShort$ map, shape styling, conditional formatting, $ProductName$, Infragistics, $PlatformShort$ マップ, 図形スタイル, 条件付き書式, インフラジスティックス
mentionedTypes: ['XamGeographicMap']
_language: ja
---
# $PlatformShort$ シェイプ スタイリング ユーティリティ

リソース トピックは、$PlatformShort$ `XamGeographicMap` コンポーネントで `GeographicShapeSeries` の UI 要素のスタイリングに役立つユーティリティの実装を提供します。
